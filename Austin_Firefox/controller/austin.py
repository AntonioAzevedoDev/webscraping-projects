...
from Austin_Firefox.data import pathandcredentials as pc
...
def save_ratings(browser, overwrite=False):
    ...
    header = [col.text for col in
              browser.find_element(By.CSS_SELECTOR, '#UpdatePanel1 > table:nth-child(2) > thead:nth-child(1) > tr:nth-child(1)').find_elements(By.TAG_NAME,'th')]
    assert len(header) == 4, f"expected 4 cols, got {len(header)}"
    payload = []
    filepath = ''
    body = browser.find_element(By.CSS_SELECTOR, "#tableRating > tbody:nth-child(1)")
